...
export function Homepage(){
    ...
    const [pratileira, setPratileira] = useState([]);

    const moverLivro = (favoritos) => {
      // Remove o livro de livrosDisponiveis
      setFavoritos((prev) =>
        prev.filter((item) => item.id !== favoritos.id)
      );
                    ...
                    <div className="popup" onClick={fecharPopup2}>
                      <div className="popup-conteudo" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="inf-Livro">
                          ...
                          <div className="dados-livro">
                            <p><strong>Autor(es):</strong> {livroSelecionado2.authors?.join(", ")}</p>
                            <p><strong>Editora:</strong> {livroSelecionado2.publisher}</p>
                            <p><strong>Data de publicação:</strong> {livroSelecionado2.publishedDate}</p>
                            <p><strong>Idioma:</strong> {livroSelecionado2.language}</p>
                            <p><strong>Categorias:</strong> {livroSelecionado2.categories?.join(", ")}</p>
                          </div>
                        </div>
                        <p><strong>Descrição:</strong> {livroSelecionado2.description}</p>
                        <a href={livroSelecionado2.infoLink} target="_blank" rel="noopener noreferrer">
                          Mais informações
                        </a>
                        <button className="ler-livro" onClick={() => { moverLivro(livroSelecionado2);}}>
                          Finalizado
                        </button>
                      </div>
                    </div>
